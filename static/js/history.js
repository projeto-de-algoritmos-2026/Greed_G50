(() => {
    ...
    function render(){ const container = document.getElementById('historyList'); const h = load(); if(!container) return; if(h.length===0){ container.innerHTML='<div>Nenhum registro ainda.</div>'; return; } container.innerHTML = h.map(it=>{ const date = new Date(it.ts); const meta = it.meta && it.meta.coins ? ` — Moedas: ${it.meta.coins.join(', ')}` : ''; return `<div style="padding:8px;border-bottom:1px solid #eee;"><div><strong>${date.toLocaleString()}</strong>${meta}</div><div style="font-size:0.95em">Nível ${it.level} — +${it.score} pts — tempo ${it.time}s — total ${it.totalScore} — ${it.optimal? 'Ótima' : 'Não'}</div></div>`; }).join(''); }
